...
export async function listen() {
  ...
  ws.onmessage = async (message) => {
    ...
    if (data.topic === "confirmation") {
      //console.log("Transaction Confirmed: Hash ", data.message.hash)
      ...
      if (subtype === "send") {
        if (!known_representatives.includes(representative) && !non_asset_representatives.includes(representative)) {
          //console.log("Send, and possibly asset representative.");
          ...
          //console.log("Is asset rep");
          if (minted_nft.owner !== block.block_account) {
            //Not owner
            return; 
          }
          // Order a recrawl
          console.log("Ordering recrawl", minted_nft.mint_hash);
